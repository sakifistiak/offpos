<?php
    ...
    function FCPFW_cart_create() {
        ...
        <div class="fcpfw_container_main" >
	        <div class="fcpfw_container" >
	            <?php
		        	$fcpfw_prodslider_enable_disable = $fcpfw_comman['fcpfw_prodslider_enable_disable']; 
		        
		        	if ( $fcpfw_prodslider_enable_disable ==='side_cart' ) : 
		            	?>
		        		<div class="btn-side_cart_open">
		                	<button id="fcpfw-sidebar-open" class="fcpfw-sidebar-open" aria-label="Show suggestions">
		                		<?php if($fcpfw_comman['fcpfw_cart_open_from'] == "left"){ ?>
                    	...
                    	<div class="top_fcpfw_bottom">
                        	<?php 
	                            ...
	                            // $WC_Cart =  WC()->cart->get_cart();;
	                           	$shipping_total = $result;

	                            $fcpfw_subtotal = WC()->cart->get_subtotal();

	                            $carta = $shipping_total - $fcpfw_subtotal;

	                            $fcpfw_get_totals = WC()->cart->get_totals();
	                           

	                            $fcpfw_cart_subtotal2 = $fcpfw_get_totals['subtotal'];

	                            $tax_class = 'standard';

	                            // Get the tax rate for the specified tax class
	                            $tax_rates = WC_Tax::get_rates_for_tax_class($tax_class);

	                            // Default values in case the tax rate is not found
	                            $tax_rate = 0;
	                            $new_width = 0;

	                            // Check if tax rates exist for the specified tax class
	                            if (!empty($tax_rates)) {
	                                // Get the first tax rate
	                                $first_tax_rate = reset($tax_rates);

	                                // Check if the first tax rate is an object
	                                if (is_object($first_tax_rate) && property_exists($first_tax_rate, 'tax_rate')) {
	                                    $tax_rate = $first_tax_rate->tax_rate;
	                                    $new_width = ($tax_rate / 100) * $fcpfw_cart_subtotal2;
	                                }
	                            }

	                            $carta2 = $carta - $new_width;
	                            // echo "<pre>";
	                            // print_r($carta2);
	                            // echo "</pre>";
	                            ...
	                            if ($carta2 < 0) {
	                                ...
	                                if ($shipping_total > $fcpfw_subtotal) {
	                                    $fcpfw_shipping_total = get_woocommerce_currency_symbol() . number_format(($carta2), 2);
                        	...
                        	<p style="color:<?php echo  esc_html($fcpfw_comman['fcpfw_header_shipping_text_color']); ?>"><?php echo esc_html($wg_prodrule_mtvtion_msg_final); ?></p>
                    	</div>
                		<?php 
                	} ?>
            	</div>
	                      	...
	                      	<div class='fcpfw_total_tr'>
	                        	<div class='fcpfw_total_tr_inner'>
	                            	<div class='fcpfw_total_label'>
	                                	<span><?php echo esc_html($fcpfw_comman['fcpfw_subtotal_txt']); ?></span>
	                            	</div>
	                            	<?php
		                            ...
		                            <?php if ($fcpfw_comman['fcpfw_total_shipping_option']== "yes" ) { ?>
		                                <div class='fcpfw_total_label'>
		                                    <span><?php echo esc_html($fcpfw_comman['fcpfw_shipping_text_trans']); ?></span>
		                                </div>
		                                <div class='fcpfw_total_amountt'>
		                                    <span class='fcpfw_fragtotall'><?php echo  esc_html($fcpfw_shipping_total); ?></span>
		                                </div>
		                            <?php } ?>
	                            	<?php  if ($fcpfw_comman['fcpfw_total_tax_option'] == "yes" ) { ?>
		                                <div class="fcpfw_total_label">
		                                     <span><?php echo esc_html($fcpfw_comman['fcpfw_apply_taxt_testx']); ?></span>
		                                </div>
	                            	...
	                            	<?php } ?>
		                            <?php  if ($fcpfw_comman['fcpfw_discount_show_cart']== "yes" ) { ?>
		                                <div class="fcpfw_oc_discount_oc">
			                                <?php if(($fcpfw_cart_discount) != 0) { ?>
			                                    <div class="fcpfw_discount_label">
			                                        <span><?php echo esc_html($fcpfw_comman['fcpfw_discount_text_trans']); ?></span>
			                                    </div>
			                                    <div class="fcpfw_discount_innwer_full">
			                                        <span class='fcpfw_discount_full'><?php echo get_woocommerce_currency_symbol().number_format( $fcpfw_cart_discount, 2); ?></span>
			                                    </div>
			                                <?php } ?>
			                             </div>
		                            <?php } ?>
	            ...
	            ?>
 				...
 				<?php
			        ...
			        if ($showSlider == 'true') {
                        ...
                        if(($fcpfw_comman['fcpfw_select_recent_related'] == 'recent' && !empty($productsa)) || $fcpfw_comman['fcpfw_select_recent_related'] == 'related') {
				            if(wp_is_mobile()) {
	        	...
	        	if ( $fcpfw_prodslider_enable_disable ==='side_cart' ) : 
			        ...
			        <div class="fcpfw_container_product">
		                ...
		                <div class="fcpfw_container_product_inner">
		                    <?php fcpfw_slider_button_product(); ?>         
		                </div>
    ...
    function FCPFW_cart_empty_value(){
        ...
        if($fcpfw_comman['fcpfw_cart_empty'] == "yes" ){
            if ( WC()->cart->is_empty() ) { 
                if($fcpfw_comman['fcpfw_show_cart_icn'] == "yes"){ 
                    if($fcpfw_comman['fcpfw_all_pages'] == "yes"){
                        echo FCPFW_cart_basket();
                    }else{
                        if ($fcpfw_comman['fcpfw_display_home_page'] == "yes") {
                            if (is_home()) {
                                echo FCPFW_cart_basket();
                            }
                        }
                        if($fcpfw_comman['fcpfw_display_shop_page'] == "yes"){
                            if(is_shop()){
                                echo FCPFW_cart_basket();
                            }
                        }
                        if($fcpfw_comman['fcpfw_display_product_page'] == "yes"){
                            if(is_product()){
                                echo FCPFW_cart_basket();
                            }
                        }
                        if($fcpfw_comman['fcpfw_display_cart_page'] == "yes"){
                            if(is_cart()){
                                echo FCPFW_cart_basket();
                            }
                        }
                        if($fcpfw_comman['fcpfw_display_checkout_page'] == "yes"){
                            if(is_checkout()){
                                echo FCPFW_cart_basket();
                            }
                        }
                        if ($fcpfw_comman['product_cat_page'] == "yes") {
                            if (is_product_category()) {
                                echo FCPFW_cart_basket();
                            }
                        }
                        if ($fcpfw_comman['product_tag_page'] == "yes") {
                            if (is_product_tag()) {
                                echo FCPFW_cart_basket();
                            }
                        }
        ...
        }elseif($fcpfw_comman['fcpfw_cart_empty'] == "no" ){
            if($fcpfw_comman['fcpfw_show_cart_icn'] == "yes"){ 
                if($fcpfw_comman['fcpfw_all_pages'] == "yes"){
                    echo FCPFW_cart_basket();
                }else{
    ...
    function FCPFW_cart_basket(){
        ...
        $style="";
        if ( $fcpfw_comman['fcpfw_cart_empty'] == "yes" && WC()->cart->is_empty() ) { 
            $style = 'style="display: none"';
        }
        ?>
        <div class="fcpfw_cart_basket" <?php echo $style; ?>>
    ...
    function FCPFW_craete_cart(){
        ...
        <style>
            ...
            <?php if($fcpfw_comman['fcpfw_cart_height']=='auto'){ ?>
	            .fcpfw_container{
	                top:auto;
	                max-height: 100%;
	            }
            ...
            .fcpfw_cart_basket{
                <?php if($fcpfw_comman['fcpfw_basket_position'] == "top"){ ?>
                	top: 15px;
                ...
                <?php } else { ?>
                	right: 15px;
                <?php } if($fcpfw_comman['fcpfw_basket_shape'] == "round"){ ?>
                    border-radius: 100%;
                <?php } else { ?>
                    border-radius: 10px;
                <?php } if($fcpfw_comman['fcpfw_cart_show_hide'] == "fcpfw_cart_hide"){ ?>
        ...
        <?php
        $fcpfw_page_ids = explode(",",$fcpfw_comman[ 'fcpfw_on_pages']);
        $fcpfw_crnt_page = get_the_ID();
        if (!in_array($fcpfw_crnt_page, $fcpfw_page_ids)) {
            if(wp_is_mobile() ){
    ...
    function FCPFW_cart_count_fragments( $fragments ) {
        ...
        //$fragments['div.fcpfw_cart_basket'] = FCPFW_cart_empty_value();
        ...
        ob_start();
        if($fcpfw_comman['fcpfw_cart_empty'] == "yes" && (WC()->cart->cart_contents_count == 0)){?>
        ...
        $fcpfw_prodslider_enable_disable = $fcpfw_comman['fcpfw_prodslider_enable_disable']; 
        if ( $fcpfw_prodslider_enable_disable ==='slider' ) : 
        	?>
	        ...
	        <script type="text/javascript">
	            var leftArrow = '<?php echo FCPFW_PLUGIN_DIR; ?>' + '/assets/images/left-arrow.svg';
	            var rightArrow = '<?php echo FCPFW_PLUGIN_DIR; ?>' + '/assets/images/right-arrow.svg';
    ...
    function fcpfw_remove_applied_coupon_ajax_call_func() {
        ...
        if(WC()->cart->remove_coupon( $code )) {
            echo esc_html($fcpfw_coupon_removed_suc_txt);
        }
        WC()->cart->calculate_totals();
        WC()->cart->maybe_set_cart_cookies();
        WC()->cart->set_session();
