...
class Subscribers_Groups_List_Table extends \WP_List_Table {
	...
	function process_bulk_action() {
		...
		// Search action
		if ( isset( $_GET['s'] ) ) {
			$prepare     = $this->db->prepare( "SELECT * from `{$this->tb_prefix}gwebsms_subscribes_group` WHERE name LIKE %s", '%' . $this->db->esc_like( $_GET['s'] ) . '%' );
		...
		if ( 'bulk_delete' == $this->current_action() ) {
			foreach ( $_GET['id'] as $id ) {
				$this->db->delete( $this->tb_prefix . "gwebsms_subscribes_group", array( 'ID' => $id ) );
			}
		...
		if ( 'delete' == $this->current_action() ) {
			$this->db->delete( $this->tb_prefix . "gwebsms_subscribes_group", array( 'ID' => $_GET['ID'] ) );
			$this->data  = $this->get_data();
			$this->count = $this->get_total();
	...
	function get_data( $query = '' ) {
		...
		if ( ! $query ) {
			$query = 'SELECT * FROM `' . $this->tb_prefix . 'gwebsms_subscribes_group` LIMIT ' . $this->limit . ' OFFSET ' . $page_number;
		} else {
	...
	function get_total( $query = '' ) {
		if ( ! $query ) {
			$query = 'SELECT * FROM `' . $this->tb_prefix . 'gwebsms_subscribes_group`';
		}
		$result = $this->db->get_results( $query, ARRAY_A );
		$result = count( $result );
