    ...
    <div id="wrapper" class="m-auto border-2s-e4e5ea br-5 p-15">
        ...
        <div class="text-center">
            ...
            <?php if ($outlet_info->email) { ?>
                <p class="f-w-500 color-71 font-size-13"><?php echo lang('email'); ?>:
                    <?php echo escape_output($outlet_info->email); ?></p>
            <?php } ?>
            <?php if ($outlet_info->phone) { ?>
                <p class="f-w-500 color-71 font-size-13"><?php echo lang('phone'); ?>:
                    <?php echo escape_output($outlet_info->phone); ?></p>
            <?php } ?>
            ...
            <div class="d-flex justify-content-between">
                <div>
                    ...
                    <?php if ($customer_info->gst_number && $s_status != 'Bangladesh') { ?>
                        <p class="f-w-500 color-71 font-size-13">
                            <span class="f-w-600"><?php echo $inv_config->customer_tax_number_label; ?>:</span>
        ...
        <div class="tharmal_table">
            <table class="table w-100 mt-20">
                        ...
                        foreach ($sale_object->items as $row):
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    echo escape_output($row->alternative_name) ? ' (' . $row->alternative_name . ')' : '';
                                    ?>
                                    ...
                                    <?php if ($inv_config->show_product_imei_serial_number == 'Yes') { ?>
                                        <?php if (($row->item_type == 'IMEI_Product' || $row->item_type == 'Serial_Product' || $row->item_type == 'Medicine_Product') && $row->expiry_imei_serial) { ?>
                                            <p class="short_note"><?php echo checkItemShortType($row->item_type) ?>:
                                                <?php echo trim($row->expiry_imei_serial); ?></p>
                                        <?php }
                                    } ?>
                                    ...
                                    <?php if ($row->warranty != 0 && $inv_config->show_warranty_expiry_date == 'Yes') { ?>
                                        <p class="text-muted short_note">
                                            <?php echo lang('warranty_expire'); ?>:
                                            <?php echo escape_output($row->warranty) . ' ' . $row->warranty_date ?>            <?php echo escape_output($row->warranty) > 3 ? 's' : '' ?>
                                        </p>
                                        <p class="text-muted short_note">
                                            <?php echo lang('will_expire'); ?>
                                            <?= date($this->session->userdata('date_format'), strtotime(dateMonthYearFinder($row->warranty, $warranty_date, $sale_object->sale_date))) ?>
                                        </p>
                                    <?php } ?>
                                    <?php
                                    $guarantee_date = '';
                                    ...
                                    <?php if ($row->guarantee != 0 && $inv_config->show_guarantee_expiry_date == 'Yes') { ?>
                                        <p class="text-muted short_note">
                                            <?php echo lang('guarantee_expire'); ?>:
                                            <?php echo escape_output($row->guarantee) . ' ' . $row->guarantee_date ?>            <?php echo escape_output($row->guarantee) > 3 ? 's' : '' ?>
                                        </p>
                                        <p class="text-muted short_note">
                                            <?php echo lang('will_expire'); ?>
                                            <?= date($this->session->userdata('date_format'), strtotime(dateMonthYearFinder($row->guarantee, $guarantee_date, $sale_object->sale_date))) ?>
                                        </p>
                                    <?php } ?>

                                    <?php if ($inv_config->show_product_image == 'Yes' && $row->photo) { ?>
                                        <img src="<?php echo base_url(); ?>uploads/items/<?php echo $row->photo; ?>"
                                            alt="Product Image" width="80" height="80">
                                    <?php } ?>

                                </td>
                                <td><?= getAmtCustom($ln_text == "bangla" ? banglaNumber($row->menu_unit_price) : $row->menu_unit_price) ?>
                                </td>
                                <td>
                                ...
                                <td class="text-right">
                                    <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($row->qty * $row->menu_unit_price) : $row->qty * $row->menu_unit_price); ?>
                                </td>
                            </tr>
                                ...
                                foreach ($combo_items as $k => $combo) {
                                    if ($combo->show_in_invoice == 'Yes') {
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($combo->combo_item_price) : $combo->combo_item_price) ?>
                                            </td>
                                            <td><?php echo $combo->combo_item_qty ?></td>
                                            <td class="text-right">
                                                <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber((int) $combo->combo_item_qty * (int) $combo->combo_item_price) : (int) $combo->combo_item_qty * (int) $combo->combo_item_price) ?>
                                            </td>
                                        </tr>
                                    <?php }
                                }
                ...
                <tfoot>
                    <tr>
                        <th></th>
                        <th colspan="2" class="text-right pr-10">
                            <?php
                            if ($s_status == 'Bangladesh') {
                        ...
                        <th><?= ($ln_text == "bangla" ? banglaNumber($totalItems) : $totalItems) ?>
                            (<?php echo ($ln_text == "bangla" ? banglaNumber($totalQty) : $totalQty) ?>)</th>
                        <th class="text-right">
            ...
            <div>
                ...
                if ($sale_object->previous_due > 0) {
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <p class="font-size-13">
                            <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($inv_prev_due) : $inv_prev_due) ?>
                            (Debit)</p>
                    </div>
                ...
                <?php } else { ?>
                        <div class="d-flex justify-content-between">
                            ...
                            <p class="font-size-13">
                            <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($sale_object->previous_due) : $sale_object->previous_due) ?>
                            </p>
                        </div>
                <?php } ?>
                <?php
                ...
                <?php if ($tax) {
                    ...
                    foreach ($tax as $t) {
                        if ($t->tax_field_amount > 0) {
                            ...
                            ?>
                            <div
                                class="d-flex justify-content-between border-bottom-dotted-gray <?php echo escape_output($i) == 1 ? 'border-top-dotted-gray' : '' ?>">
                ...
                if ($sale_object->total_payable) {
                    ...
                    <div class="d-flex justify-content-between border-bottom-dotted-gray">
                        ...
                        ?>
                        <p class="font-size-13">
                            <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($totalpayable) : $totalpayable) ?></p>
                    </div>
                    <?php
                }
                ?>
                ...
                if ($sale_object->paid_amount) {
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <p class="font-size-13">
                            <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($sale_object->paid_amount) : $sale_object->paid_amount) ?>
                        </p>
                    </div>
                    <?php
                }
                ...
                if ($sale_object->due_amount > 0) {
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <p class="font-size-13">
                            <?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber($sale_object->due_amount) : $sale_object->due_amount) ?>
                        </p>
                    </div>
                <?php } ?>
                ...
                if ($sale_object->due_amount < 0) {
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <p><?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber(absCustom($due_reveive) ? absCustom($due_reveive) : 0) : (absCustom($due_reveive) ? absCustom($due_reveive) : 0)); ?>
                        </p>
                    </div>
                    <div class="d-flex justify-content-between">
                        <p class="f-w-600 font-size-13">
                        ...
                        <p><?php echo getAmtCustom($ln_text == "bangla" ? banglaNumber(absCustom($advance_receive) ? absCustom($advance_receive) : 0) : (absCustom($advance_receive) ? absCustom($advance_receive) : 0)); ?>
                        </p>
                    </div>
                <?php } ?>
                ...
                <?php if ($inv_config->show_payment_method == 'Yes') { ?>
                    ...
                    $salePaymentDetails = isset($payment_methods_override) && $payment_methods_override ? $payment_methods_override : salePaymentDetails($sale_object->id, $outlet_id);
                                ...
                                <p class="f-w-600 font-size-13">
                                    ...
                                    if ($p_name->payment_details != '') {
                                        foreach ($payment_details as $key => $details) {
                                            ?>
                                            <span
                                                class="font-size-10"><?php echo $details != '' ? ($key === array_key_last($payment_details) ? $details : $details . ',') : ''; ?>
                                            </span>
                                        <?php }
                                    } ?>
                                </p>
            ...
            <div class="d-flex justify-content-center pt-30">
                <div>
                    <img width="80" height="80"
                        src="<?php echo base_url() ?>uploads/qr_code/<?php echo escape_output($sale_object->id) ?>.png">
                </div>
            </div>
        <?php } ?>
        <div class="d-flex justify-content-center pt-30">
