...
<head>
    ...
    <style>
        body {
            font-family: 'DejaVu Sans', 'Segoe UI Symbol', 'Noto Sans Bengali', Arial, sans-serif;
            font-size: 12px;
            color: #000;
            margin: 0
        }
...
<body>
    <div class="page">
        <div class="logo-wrap">
            <?php if ($logo) { ?><img class="logo"
                    src="<?php echo base_url() . 'uploads/site_settings/' . escape_output($logo); ?>"><?php } ?>
        </div>
            ...
            <tr>
                ...
                <td style="width:42%;" class="right small">
                    ...
                    <div><span class="bold">Date:</span>
                        <?php echo date('d/m/Y', strtotime($sale_object->sale_date)); ?></div>
                </td>
            </tr>
        </table>
        ...
        <table class="items mt15">
            ...
            <tbody>
                ...
                foreach (($sale_object->items ?? []) as $row):
                    ...
                    <tr>
                        <td><?php echo $i++; ?></td>
                        <td><?php echo escape_output(getItemAndParntName($row->food_menu_id)); ?><?php echo !empty($row->alternative_name) ? ' - ' . escape_output($row->alternative_name) : ''; ?>
                        </td>
                        <td class="right"><?php echo $money($row->menu_unit_price); ?></td>
                        <td class="right">
                            <?php echo ($ln_text == 'bangla' ? banglaNumber($row->qty) : $row->qty) . ' ' . unitName(getSaleUnitIdByIgId($row->food_menu_id)); ?>
                        </td>
                        <td class="right"><?php echo $money($row->qty * $row->menu_unit_price); ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
        ...
        <table class="summary mt10">
            <tr>
                <td style="width:70%;" class="bold">Previous Balance</td>
                <td class="right"><?php echo $money($sale_object->previous_due ?? 0); ?></td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="bold">Charge<br>(Delivery)</td>
                <td class="right"><?php echo $money($sale_object->delivery_charge); ?></td>
            </tr>
            <tr>
                <td class="bold">Total Payable</td>
                <td class="right"><?php echo $money($sale_object->total_payable); ?></td>
            </tr>
            <tr class="line">
                <td class="bold">Paid Amount</td>
                <td class="right"><?php echo $money($sale_object->paid_amount); ?></td>
            </tr>
            <tr>
                <td class="bold">Due Amount</td>
                <td class="right"><?php echo $money($sale_object->due_amount); ?></td>
            </tr>
            <?php if (!empty($sale_object->partner_name)) { ?>
                <tr>
                    <td class="bold">Delivery Partner</td>
                    <td class="right"><?php echo escape_output($sale_object->partner_name); ?></td>
                </tr><?php } ?>
            <?php if (!empty($payment_rows)) { ?>
                <tr class="line">
                    <td class="bold">Payment Method</td>
                    <td></td>
                </tr><?php foreach ($payment_rows as $p) { ?>
                    <tr>
                        <td class="bold"><?php echo escape_output($p->payment_name); ?></td>
                        <td class="right"><?php echo $money($p->amount); ?></td>
                    </tr><?php } ?><?php } ?>
            <tr class="line">
                <td class="center bold" colspan="2">Given Amount<br>:
                    <?php echo $money($sale_object->given_amount ?? 0); ?></td>
            </tr>
            <tr>
                <td class="center bold" colspan="2">Change Amount<br>:
                    <?php echo $money($sale_object->change_amount ?? 0); ?></td>
            </tr>
        </table>
        ...
        <div class="mt20">
            <div class="note-red">Note: <span class="italic" style="color:#000;">Only products that are faulty or
                    incorrectly delivered by BD Baking are eligible for return.</span></div>
            <div class="italic">BD Baking is not responsible for any damage caused during courier transit,this remains
                the customer's sole responsibility.</div>
        </div>
    </div>
</body>
