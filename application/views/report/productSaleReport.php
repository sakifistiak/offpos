...
<input type="hidden" id="The_Product_field_is_required" value="<?php echo lang('The_Product_field_is_required');?>">

<link rel="stylesheet" href="<?php echo base_url(); ?>frequent_changing/css/report.css">
<script src="<?php echo base_url(); ?>frequent_changing/js/product_sale_report.js"></script>

<div class="main-content-wrapper">
    <section class="content-header">
        <div class="row justify-content-between">
            <div class="col-6 p-0">
                <h3 class="top-left-header mt-2"><?php echo lang('productSaleReport'); ?></h3>
            </div>
            <?php $this->view('updater/breadcrumb', ['firstSection'=> lang('report'), 'secondSection'=> lang('productSaleReport')])?>
        </div>
    </section>
    ...
    <div class="box-wrapper">
        ...
        <div class="report_header">
            ...
            <h5 class="outlet_info">
                <strong><?php echo lang('productSaleReport'); ?></strong>
            </h5>
        ...
        <div class="table-box">
            <div class="box-body">
                ...
                <input type="hidden" class="datatable_name"  data-filter="yes" data-title="<?php echo lang('productSaleReport'); ?>" data-id_name="datatable">
                    <table id="datatable" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th class="w-2"><?php echo lang('sn'); ?></th>
                        ...
                        <tbody>
                            <?php
                            $pGrandTotal = 0;
                            if (isset($productSaleReport)):
                                foreach ($productSaleReport as $key => $value) {
                                    $key++;
                                    ?>
                                    <tr>
                                        <td><?php echo $key; ?></td>
                                        ...
                                        <td>
                                            <?php echo escape_output($value->name) . "(" . $value->code . ")" ?> <br>
                                            <?php
                                            if($value->menu_note != ''){
                                                echo "(<strong>" . checkItemShortType($value->type) . ":</strong>" . $value->menu_note . ')';
                                            }
                                            ?>
                                        </td>
                                        <td class="text-center"><?php echo escape_output($value->totalQuantity_amount) ?> <?php echo escape_output($value->unit_name);?></td>
                                        <td class="text-center"><?php echo escape_output($value->unit_price) ?></td>
                                        <td><?php echo $value->unit_price*$value->totalQuantity_amount ?></td>
                                    </tr>
                                    <?php
                                }
                            endif;
...
<div id="product-filter" class="filter-modal">
    <div class="filter-modal-body">
        ...
        <?php echo form_open(base_url() . 'Report/productSaleReport', $arrayName = array('id' => 'productSaleReport')) ?>
        <div class="row">
            ...
            <div class="col-12 mb-2">
                <button type="submit" name="submit" value="submit" class="new-btn productSaleReport">
